# tests/test_comprehensive_alias_analysis.py
...
def _verify_city_exists(city: str, country: str, alias: str = "") -> tuple[bool, str]:
    ...
    try:
        ...
        print(f"  {alias:20} | {country:25} | {city:35} | {is_valid:5} | {location_type}")
        
        return is_valid, location_type
    except Exception as e:
        # If verification fails, print error and assume it might be real
        print(f"  {alias:20} | {country:25} | {city:35} | ERROR | API_ERROR: {e}")
        return True, "API_ERROR"


def test_comprehensive_alias_analysis():
    """Comprehensive analysis of ALL GDACS aliases for city data quality."""
    
    import sys
    comprehensive_mode = True  # Force comprehensive mode to get all data with eventtype/eventid
    
    # All GDACS aliases discovered from exhaustive search
    ...
    print(f"Discovered {len(all_gdacs_aliases)} GDACS aliases: {all_gdacs_aliases}")
    
    # Get disaster data with comprehensive alias processing
    disasters, total_red = recon(debug=False, comprehensive_aliases=True)
    
    # Debug: Show sample disaster structure
    if disasters:
        print(f"Sample disaster keys: {list(disasters[0].keys())}")
        # Show first few disasters to see the structure
        for i, d in enumerate(disasters[:3]):
            print(f"Disaster {i}: {dict(list(d.items())[:5])}")  # Show first 5 key-value pairs
    
    # Discover aliases actually present in the data
    actual_aliases = set()
    for d in disasters:
        alias = d.get('alias_source', '')
        if alias:
            actual_aliases.add(alias)
    
    print(f"Aliases found in data: {len(actual_aliases)}")
    print(f"Actual aliases: {sorted(actual_aliases)}")
    print(f"Expected aliases not found: {set(all_gdacs_aliases) - actual_aliases}")
    ...
    if comprehensive_mode:
        ...
        # Sample unique countries per alias for current aliases
        sampled_by_alias = {}
        for d in disasters:
            alias = d.get('alias_source', '')
        ...
        for alias, countries_dict in sampled_by_alias.items():
            for country, city in countries_dict.items():
                disasters_to_test.append({
                    'eventtype': 'UNKNOWN',  # Not available in systematic sampling
                    'eventid': 'UNKNOWN',    # Not available in systematic sampling
                    'alias': alias,
                    'country': country,
                    'city': city
                })
        
        print(f"Testing {len(disasters_to_test)} systematic samples across current aliases...")
    
    # Verify each disaster and collect results
    alias_quality = {}
    total_tests = 0
    ...
    for d in disasters_to_test:
        ...
        if not alias or not country or not city:
            continue
        ...
        eventtype = d.get('type') or d.get('eventtype') or 'UNKNOWN'
        eventid = d.get('eventid') or 'UNKNOWN'
        verification_results.append([eventtype, eventid, alias, country, city, is_valid, location_type])
    ...
    print(f"✓ Real cities: {sum(r[5] for r in verification_results)}")
    print(f"✗ Not cities: {len(verification_results) - sum(r[5] for r in verification_results)}")
    print(f"Total tested: {total_tests}")
    print(f"Unique aliases analyzed: {len(alias_quality)}")
    
    # Comprehensive alias quality percentages
    print(f"\nComplete Alias Quality Percentages:")
    print(f"  {'ALIAS':20} | {'TOTAL':8} | {'VERIFIED':9} | {'PERCENTAGE':12}")
    print(f"  {'-'*20} | {'-'*8} | {'-'*9} | {'-'*12}")
    
    # Sort aliases by percentage (best to worst)
    sorted_aliases = sorted(
        alias_quality.items(), 
        key=lambda x: (x[1]['verified'] / x[1]['total'] * 100) if x[1]['total'] > 0 else 0,
        reverse=True
    )
    
    for alias, stats in sorted_aliases:
        total = stats['total']
        verified = stats['verified']
        percentage = (verified / total * 100) if total > 0 else 0
        print(f"  {alias:20} | {total:8} | {verified:9} | {percentage:11.1f}%")
    
    # Write comprehensive results to CSV
    csv_filename = 'alias_analysis.csv' if comprehensive_mode else 'comprehensive_alias_analysis.csv'
    ...
    with open(csv_file, 'w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow(['COMPREHENSIVE ALIAS ANALYSIS'])
        ...
        writer.writerow(['Unique Aliases', len(alias_quality)])
        writer.writerow(['Real Cities', sum(r[5] for r in verification_results)])
        writer.writerow(['Not Cities', len(verification_results) - sum(r[5] for r in verification_results)])
        writer.writerow([])
        
        # Write complete alias quality percentages
        writer.writerow(['COMPLETE ALIAS QUALITY PERCENTAGES'])
        ...
        writer.writerow(['EVENTTYPE', 'EVENTID', 'ALIAS', 'COUNTRY', 'CITY', 'VALID', 'LOCATION_TYPE'])
        writer.writerows(verification_results)
    
    print(f"\nComprehensive analysis written to: {csv_file}")
